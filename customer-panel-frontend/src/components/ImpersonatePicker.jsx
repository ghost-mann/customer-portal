...
export default function ImpersonatePicker() {
  const { ctx, impersonate, setImpersonate } = useStore();
  ...
  // Don't render anything for non-staff users.
  if (!ctx?.is_staff) return null;

  const label = impersonate ? `Viewing as: ${impersonate}` : `Viewing as: ${ctx.customer || '—'}`;

  return (
    <div className="imp-wrap" ref={wrapRef}>
      <button className="imp-btn" onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }} title="Staff: switch customer">
        ...
        <div className="imp-drop">
          ...
          {impersonate && (
            <div className="imp-item imp-clear" onClick={() => { setImpersonate(null); setOpen(false); }}>
              <Icon name="close" />
              <span>Stop impersonating (back to default)</span>
            </div>
          )}
